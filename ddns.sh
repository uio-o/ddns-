#!/bin/bash
# 修复核心版 Cloudflare DDNS (修复多网卡IP抓取失败)
...
manage_domains() {
    ...
    echo -e "${green}[+] 域名配置已就绪。${none}"
    sleep 2
}

force_run() {
    echo -e "${cyan}正在前台强制运行解析检查...${none}"
    /usr/bin/ddns cron
    echo -e "${green}执行完毕！如果有日志未输出，说明IP无变化...${none}"
    sleep 2
    menu
}
...
menu() {
    ...
    case $choice in
        ...
        3) 
           source "$config_file"
           echo -e "\n[-] 已设定的 IPv4 域名: ${Domains[*]}"
           echo -e "[-] 已设定的 IPv6 域名: ${Domains6[*]}"
           read -p "按回车返回..."
           menu ;;
        0) uninstall_ddns ;;
        99) exit 0 ;;
